...
| **`chrome-devtools-mcp`** | per its own docs                                          | **Every performance claim in this repo is still an estimate**, but the reason has changed. This row used to say profiling was impossible because the browser here could not reach the network. That is no longer the obstacle: Tahap 12 drove Chromium against a local production build for every motion measurement in it, and profiling a local build is exactly what is needed — no egress required. What is missing is the work, not the capability. |
