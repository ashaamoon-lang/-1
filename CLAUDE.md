...
### Which of these a gate can actually see

Generated by `lib/scripts/rule-coverage.ts`; `bun run rules:coverage` rewrites
it and `rule-coverage.test.ts` fails if it drifts. **Do not edit by hand.**

A rule with no instrument is not a lesser rule — it is one this project has to
keep by hand, and saying which those are is the point. Tahap 78 found rule #1
enforced in one directory out of six and proved it by putting a violation in a
shipped component: the whole suite passed.

<!-- rule-coverage:start -->

```
 1  motion-rules #1 (CSS, repo-wide) + GSAP dialect; vendor-rules
 2  motion-rules #2
 3  motion-rules #8 (partial)
 4  motion-rules #4
 5  motion-rules reduced-motion contract
 6  taste-rules; vendor-rules #6
 7  nothing can fail on it
 8  motion-rules #8; token-rules; scale-rules
 9  token-rules #9
10  token-rules; vendor-rules #10
11  token-rules #11 (partial)
12  scale-rules
13  webgl-budget; route-budget
14  webgl-budget
15  material-layer (partial)
16  vendor-rules provenance
17  vendor-rules #17
18  nothing can fail on it
19  nothing can fail on it
20  nothing can fail on it
21  nothing can fail on it

21 rules — 16 with a gate that can fail, 5 without (#7, #18, #19, #20, #21)
3 covered in part only (#3, #11, #15); each says where its coverage stops
```
